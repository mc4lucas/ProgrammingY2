...
public class DoublyLinkedList<E> {
    ...
    public Node<E> findNode(E e){
        if(!isEmpty()){
            int pos = 0; //Iterator
            Node<E> nextNode = header.getNext(); // This will start the link

            while(pos < size){
                if(nextNode.getElement() == e){return nextNode;} //Checks if the current node contains an element equal to the sent node
                nextNode = nextNode.getNext(); //Increment
                pos++; //Incrementing
    ...
    public String toString(){
        int pos = 0; //Iterator
        ArrayList<E> al = new ArrayList<>(); //Arraylist to store the nodes in
        Node<E> nextNode = header.getNext(); // This will start the link

        while(pos < size){
            al.add(nextNode.element); //Adding the noes to a list
            nextNode = nextNode.getNext(); //Retrieving the next node in the link
            pos++; //Incrementing
